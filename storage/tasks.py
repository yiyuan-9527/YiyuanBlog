...
def storage_plan_check() -> str:
    ...
    expired_storage_instances = (
        Storage.objects.filter(
            plan_expire_at__isnull=False,
            plan_expire_at__lte=datetime.now(utc=True),
        )
        .exclude(
            plan_name=Storage.PlanChoices.FREE,
        )
        .select_related('user')
    )
    ...
    # 建立一個 group, 每個 entry 是一個獨立的 downgrade_user_storage_plan 任務
    # .s() 是簽名, 用於異步呼叫任務
    job = group(
